...
#making window flixable for all res's
...
#creating pm and am button's functions
def pm () :
    global x
    global r
    global a
...
def set () :
    ...
    while True :
        global hr , min , sec , x , y , user_time , z , r , set_btn , p , a , m , d , s , Flag , state
        hr = hr_entry.get()
        min = min_entry.get()
        sec = sec_entry.get()
        if y == 1 :
        ...
        if a == 0 :
            if hrs < 12 :
                user_time = f"{hr}:{min}:{sec}"
        if p == 1 and current_hrs == 12:
                user_time = f"{str(int(hr) + 12)}:{min}:{sec}"
        if hrs == 12 and x == 1 or p == 1 :
            user_time = f"12:{min}:{sec}"
        if int(hr) < 12 and p == 1  :
            user_time = f"{str(int(hr) + 12)}:{min}:{sec}"
            hrs += 12
        if a == 1 or x == 1 or p == 1 :
            state = "PM"
        else:
            state = "AM"
        blank_label = customtkinter.CTkLabel(root, text="aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", text_color="#242424")
        blank_label.place(x=125, y=500)
        count_label = customtkinter.CTkLabel(root, text_color="green", text=f"Alarm will ring at {hrs} : {mins} : {secs} {state}" )
        count_label.place(x=170, y=500)
        if z == 1 :
            count_label.destroy()
        print(user_time,current_time)
        if user_time == current_time:
            stop_btn.destroy()
            playsound("Elements\\Sound\\Alarm.wav" )
...
am_check = customtkinter.CTkRadioButton(root , text = "AM" ,width = 5 , value = 0 , variable = x , command = am)
